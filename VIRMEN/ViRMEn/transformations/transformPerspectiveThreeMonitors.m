function [coord3new] = transformPerspectiveThreeMonitors(coord3)
...
ncols = size(coord3,2);
% outputs
coord3new = zeros(3, ncols, 3);


% Create 3 different viewpoints for 3 monitors (at cardinal points of compass)
% ----------------------------------------------------------------------------
coord3 = repmat( coord3, [1 1 3] );   % First, replicate the input coords x3.
...
[xSign,zSign] = deal( ones(1,ncols,3) .* -1 );
xSign( coord3(1,:,:)>0 ) = 1;
zSign( coord3(3,:,:)>0 ) = 1;
