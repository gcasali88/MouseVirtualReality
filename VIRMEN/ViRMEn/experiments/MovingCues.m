function code = MovingCues
% MovingCues   Code for the ViRMEn experiment MovingCues.
%   code = MovingCues   Returns handles to the functions that ViRMEn
...
vr.Experiment = 'MovingCue';
[vr] = MovingCueInsertTestData(vr);
[vr] = MovingCueSelectWorld(vr);
[vr] = MovingCueTrialSettings(vr);
vr = MovingCueTrialEnvironmentSettings( vr );
...
vr = MovingCueTransportToStart(vr);
...
vr.InRewardZone = vr.position(2) < vr.RewardLocation + vr.TrialSettings.HalfCueSize && vr.position(2) > vr.RewardLocation - (vr.TrialSettings.HalfCueSize);
